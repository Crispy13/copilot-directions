---
name: work_for_goal
description: "Plan, execute, and report on the active mission goal using mission_folder, the active plan, and the stage plan."
argument-hint: "Active goal or subgoal to execute"
---
...
- `Mission goal`: the higher-level objective described in `<mission-folder>/copilot-project-plan.md`.
- `Active plan`: `<mission-folder>/copilot-active-plan.md`, which tracks the currently active goal, the working state needed to continue it, and the high-level slices or expected stages for that goal.
- `Stage plan`: `<mission-folder>/copilot-stage-plan.md`, which defines the next actionable stage to execute.
...
- First resolve the `<mission-folder>` placeholder using `../instructions/project_context.instructions.md`.
...
2. Otherwise, if session history or `<mission-folder>/copilot-active-plan.md` already shows an unresolved active goal, continue that active goal.
3. Otherwise, use the mission goal from `<mission-folder>/copilot-project-plan.md` as the active goal.
...
- If `<mission-folder>/copilot-active-plan.md` is missing, stale, or no longer matches the active goal, run a custom agent named exactly 'Planner' as subagent to produce updated active-plan content, then write that content to `<mission-folder>/copilot-active-plan.md` in the main agent.
...
1. Run a custom agent named exactly 'Plan' as subagent to update or generate the next stage plan:
  ...
  3. Use the slices or expected stages already recorded in `<mission-folder>/copilot-active-plan.md` when they are still valid. Re-slice only if the active plan is missing that structure or it is no longer valid.
  4. Produce the next stage-plan content for `<mission-folder>/copilot-stage-plan.md` and return it to the main agent.

2. Update `<mission-folder>/copilot-stage-plan.md` in the main agent with the stage-plan content returned by the `Plan` subagent.
...
- When delegating to subagents, include these:
  ...
  2. the stage plan path: `<mission-folder>/copilot-stage-plan.md` (pass it directly to the subagent)
...
- Update `<mission-folder>/copilot-active-plan.md` only when it truly needs modification.
